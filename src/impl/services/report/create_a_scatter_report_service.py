...
class CreateAScatterReportService:
    ...
    def preprocess_request_data(self):
        ...
        try:
            ...
            logger.debug(f"session_factory is of type {type(session_factory)}")
            logger.debug(f"sessionmaker_instance is of type {type(sessionmaker_instance)}")
            logger.debug(f"session is of type {type(session)}")
            ...
            try:
                ...
                report = report_repository.create_a_scatter_report(
                    user_id=user_id,
